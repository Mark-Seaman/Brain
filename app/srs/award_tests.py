...
class AwardTest(TestCase):
    ...
    def test_award(self):
        """
        Test the award data type
        """

        # Setup test records with 10 new records
        test_award()
        self.assertEqual(10,len(Award.objects.filter(name__startswith='Name ')))
        self.assertEqual(10,len(Award.objects.all()))

        # Convert to string 
        a = award_get(3)
        s = award_detail(a)
        answer = [3, u'Name 3', u'Award 3', u'', u'City 3', u'State 3', u'Zip 3', u'Phone 3',
                  0.0, u'Email 3', u'']
        self.assertEqual(s,answer)

        # Show the labels for a table
        s = award_labels
        answer =  [ 'id', 'name', 'award1', 'award2', 'city', 'state', 'zipcode', 'phone', 
