...
class KudoTest(TestCase):

    def test_kudo(self):
        """
        Test the kudo data type
        """

        # Setup test records with 10 new records
        test_kudo()
        self.assertEqual(10,len(Kudo.objects.filter(name__startswith='Name ')))
        self.assertEqual(10,len(Kudo.objects.all()))

        # Convert to string 
        a = kudo_get(3)
        s = kudo_detail(a)
        answer = [3, u'Name 3', u'Kudo 3', u'', u'City 3', u'State 3', u'Zip 3', u'Phone 3',
                  0.0, u'Email 3', u'']
        self.assertEqual(s,answer)

        # Show the labels for a table
        s = kudo_labels
        answer =  [ 'id', 'name', 'kudo1', 'kudo2', 'city', 'state', 'zipcode', 'phone', 
