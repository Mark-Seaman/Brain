...
class GroupTest(TestCase):
    ...
    def test_group(self):
        """
        Test the group data type
        """

        # Setup test records with 10 new records
        test_group()
        self.assertEqual(10,len(Group.objects.filter(name__startswith='Name ')))
        self.assertEqual(10,len(Group.objects.all()))

        # Convert to string 
        a = group_get(3)
        s = group_detail(a)
        answer = [3, u'Name 3', u'Group 3', u'', u'City 3', u'State 3', u'Zip 3', u'Phone 3',
                  0.0, u'Email 3', u'']
        self.assertEqual(s,answer)

        # Show the labels for a table
        s = group_labels
        answer =  [ 'id', 'name', 'group1', 'group2', 'city', 'state', 'zipcode', 'phone', 
