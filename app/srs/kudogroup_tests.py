...
from kudogroup import *

class KudoGroupTest(TestCase):

    def test_kudogroup(self):
        """
        Test the kudogroup data type
        """

        # Setup test records with 10 new records
        test_kudogroup()
        self.assertEqual(10,len(KudoGroup.objects.filter(name__startswith='Name ')))
        self.assertEqual(10,len(KudoGroup.objects.all()))

        # Convert to string 
        a = kudogroup_get(3)
        s = kudogroup_detail(a)
        answer = [3, u'Name 3', u'KudoGroup 3', u'', u'City 3', u'State 3', u'Zip 3', u'Phone 3',
                  0.0, u'Email 3', u'']
        self.assertEqual(s,answer)

        # Show the labels for a table
        s = kudogroup_labels
        answer =  [ 'id', 'name', 'kudogroup1', 'kudogroup2', 'city', 'state', 'zipcode', 'phone', 
