...
class IdeaGroupTest(TestCase):

    def test_ideagroup(self):
        """
        Test the ideagroup data type
        """

        # Setup test records with 10 new records
        test_ideagroup()
        self.assertEqual(10,len(IdeaGroup.objects.filter(name__startswith='Name ')))
        self.assertEqual(10,len(IdeaGroup.objects.all()))

        # Convert to string 
        a = ideagroup_get(3)
        s = ideagroup_detail(a)
        answer = [3, u'Name 3', u'IdeaGroup 3', u'', u'City 3', u'State 3', u'Zip 3', u'Phone 3',
                  0.0, u'Email 3', u'']
        self.assertEqual(s,answer)

        # Show the labels for a table
        s = ideagroup_labels
        answer =  [ 'id', 'name', 'ideagroup1', 'ideagroup2', 'city', 'state', 'zipcode', 'phone', 
