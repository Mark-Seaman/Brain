...
class CompanyTest(TestCase):
    ...
    def test_company(self):
        """
        Test the company data type
        """

        # Setup test records with 10 new records
        test_company()
        self.assertEqual(10,len(Company.objects.filter(name__startswith='Name ')))
        self.assertEqual(10,len(Company.objects.all()))

        # Convert to string 
        a = company_get(3)
        s = company_detail(a)
        answer = [3, u'Name 3', u'Company 3', u'', u'City 3', u'State 3', u'Zip 3', u'Phone 3',
                  0.0, u'Email 3', u'']
        self.assertEqual(s,answer)

        # Show the labels for a table
        s = company_labels
        answer =  [ 'id', 'name', 'company1', 'company2', 'city', 'state', 'zipcode', 'phone', 
