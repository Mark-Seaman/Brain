...
class ProviderTest(TestCase):
    ...
    def test_provider(self):
        """
        Test the provider data type
        """

        # Setup test records with 10 new records
        test_provider()
        self.assertEqual(10,len(Provider.objects.filter(name__startswith='Name ')))
        self.assertEqual(10,len(Provider.objects.all()))

        # Convert to string 
        a = provider_get(3)
        s = provider_detail(a)
        answer = [3, u'Name 3', u'Provider 3', u'', u'City 3', u'State 3', u'Zip 3', u'Phone 3',
                  0.0, u'Email 3', u'']
        self.assertEqual(s,answer)

        # Show the labels for a table
        s = provider_labels
        answer =  [ 'id', 'name', 'provider1', 'provider2', 'city', 'state', 'zipcode', 'phone', 
